                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                                        ...
                                        <tbody>
										<?php
											$sql=$link->rawQuery("select op.*,oi.*,p.product_id,p.product_name,p.product_image,p.product_price,p.product_gst,p.product_shipping_charge from order_product op,order_item oi,product p where op.order_product_id=oi.order_product_id and oi.product_id=p.product_id and op.order_product_id=?",array($_REQUEST['oid']));
											if($sql)
											{
												foreach($sql as $cat)
												{
													?>
													<tr>
														<td><img style="height: 30px;width: 30px;"  src="../images/product_image/<?php echo $cat['product_image']; ?>"></td>
														<td><?php echo $cat['product_name']; ?></td>
														<td><?php echo $cat['qty']; ?></td>
														<td><?php echo $cat['product_price']; ?></td>
														<td><?php echo sprintf("%.2f", $product_total=$cat['product_price']*$cat['qty']); ?></td>
														<td><?php echo sprintf("%.2f", $gst=$product_total*$cat['product_gst']/100); ?></td>
														<td><?php echo sprintf("%.2f", $ship=$cat['product_shipping_charge']*$cat['qty']); ?></td>
														<td><?php echo sprintf("%.2f",$product_total+$gst+$ship); ?></td>
													</tr>
													<?php
												}
											}
